...
pub async fn handle_storage(turn_data: ConversationTurn, memory_store: Arc<MemoryStore>) -> Result<()> {
    ...
    let key = format!("turn_{}", turn_data.user_query.chars().take(40).collect::<String>().replace(' ', "_"));
