...
impl Cli {
    fn get_install_dir(&self) -> Result<PathBuf> {
        let path_str = if self.install_dir.starts_with("~/") {
            match home::home_dir() {
                Some(path) => path.join(self.install_dir.strip_prefix("~/").ok_or_else(|| anyhow!("Invalid path prefix"))?),
    ...
    fn get_config_dir(&self) -> Result<PathBuf> {
        let path_str = if self.config_dir.starts_with("~/") {
            match home::home_dir() {
                Some(path) => path.join(self.config_dir.strip_prefix("~/").ok_or_else(|| anyhow!("Invalid path prefix"))?),
                None => return Err(anyhow!("Could not resolve home directory")),
            }
        } else {
...
fn stop_daemon_if_running(daemon: &DaemonInfo, runtime_dir: &Path) -> Result<()> {
    info!("Checking status for daemon: {}", daemon.bin_name);
                    ...
                    if is_process_running(pid) {
                        ...
                        match kill(Pid::from_raw(pid), Some(Signal::SIGTERM)) {
                            Ok(_) => {
                                ...
                                if is_process_running(pid) {
                                    ...
                                    if let Err(e) = kill(Pid::from_raw(pid), Some(Signal::SIGKILL)) {
                                        ...
                                        info!("  Daemon {} (PID {}) force-killed.", daemon.bin_name, pid);
                                    }
                                } else {
                                    info!("  Daemon {} (PID {}) stopped gracefully.", daemon.bin_name, pid);
                                }
                            }
                            ...
                            Err(e) => {
                                ...
                                warn!("  Attempting SIGKILL for {} (PID {})...", daemon.bin_name, pid);
                                if is_process_running(pid) { // Check again before SIGKILL
                                    if let Err(e_kill) = kill(Pid::from_raw(pid), Some(Signal::SIGKILL)) {
                                        error!("  Failed to send SIGKILL to PID {}: {}", pid, e_kill);
                                    } else {
    ...
    if pid_file_path.exists() {
        if let Err(e) = fs::remove_file(&pid_file_path) {
            warn!("  Failed to remove PID file {}: {}", pid_file_path.display(), e);
        } else {
...
fn check_and_prompt_for_config_value(
    ...
    if !config_path.exists() {
        warn!("Config file {} not found for checking value. Skipping.", config_path.display());
        return Ok(());
    }

    let config_content = fs::read_to_string(config_path)?;
    let mut config_value: toml::Value = config_content.parse()
    ...
    if needs_update {
        ...
        let mut current_table = config_value.as_table_mut()
        ...
        for (i, key) in key_path.iter().enumerate() {
            if i == key_path.len() - 1 {
                ...
                let entry = current_table.entry(key.to_string())
                    .or_insert_with(|| toml::Value::Table(Table::new()));
                current_table = entry.as_table_mut()
                    .ok_or_else(|| anyhow!("Expected intermediate key '{}' to be a table in {}", key, config_path.display()))?;
            }
        }
        ...
        info!("Updated {} in {}.", key_path.join("."), config_path.display());
    }
...
fn main() -> Result<()> {
    ...
    match &cli.command {
        Some(Commands::Install { skip_shell_wrapper, skip_manager }) => {
            install_all(&install_dir, &config_dir, &daemons, &mcp_servers, !skip_shell_wrapper, !skip_manager)?;
        }
        Some(Commands::Uninstall { force, uninstall_manager }) => {
            uninstall_all(&install_dir, &config_dir, &daemons, &mcp_servers, *force, *uninstall_manager)?;
        }
        Some(Commands::Update { skip_daemons, update_manager }) => {
            update_all(&install_dir, &config_dir, &daemons, &mcp_servers, !skip_daemons, *update_manager)?;
        }
        ...
        None => {
            ...
            install_all(&install_dir, &config_dir, &daemons, &mcp_servers, true, true)?;
        }
    }
...
fn install_all(
    ...
    install_binary("gemini-cli-bin", install_dir)?;
    for daemon in daemons {
...
fn build_binaries(build_manager: bool) -> Result<()> {
    ...
    for binary in &binaries {
        ...
        if !binary_path.exists() {
            return Err(anyhow!("Binary not found after build: {}", binary_path.display()));
        }
...
fn install_binary(bin_name: &str, install_dir: &Path) -> Result<()> {
    ...
    if !src_path.exists() {
        ...
        if dir_path.exists() {
            ...
            let entries = match fs::read_dir(dir_path) {
                Ok(entries) => entries,
                Err(e) => {
                    return Err(anyhow!("Could not read directory {}: {}", dir_path.display(), e));
                }
            };
    ...
    info!("Copying from {} to {}", src_path.display(), dst_path.display());
...
# Gemini CLI Wrapper Function End
"#, cli_bin_path.display(), config_dir.display());

    append_to_shell_config(&shell_config_path, &wrapper_content)?;
    info!("CLI wrapper function created in {}", shell_config_path.display());
    
    Ok(())
}

fn create_daemon_wrapper(daemon: &DaemonInfo, install_dir: &Path, config_dir: &Path) -> Result<()> {
    info!("Creating {} wrapper function...", daemon.function_name);
    
    let shell_config_path = get_shell_config_path()?;
    let daemon_bin_path = install_dir.join(&daemon.bin_name);
    
    // Define markers
    let start_marker = format!("# {} Management Function Start", daemon.function_name.to_uppercase());
    let end_marker = format!("# {} Management Function End", daemon.function_name.to_uppercase());
...
"#, 
    ...
    append_to_shell_config(&shell_config_path, &wrapper_content)?;
    info!("{} wrapper function created in {}", 
...
fn install_unified_config(install_dir: &Path, config_dir: &Path, mcp_servers: &[&str]) -> Result<()> {
    ...
    if !config_path.exists() {
        ...
        info!("Created default MCP server configuration at {}", mcp_config_path.display());
        
    } else {
        info!("Unified configuration already exists at {}", config_path.display());
...
fn uninstall_all(
    ...
    uninstall_binary("gemini-cli-bin", install_dir)?;
    for daemon in daemons {
    ...
    let shell_config_path = get_shell_config_path()?;
    info!("Removing shell wrapper functions from {}...", shell_config_path.display());
    remove_shell_function(&shell_config_path, 
        "# Gemini CLI Wrapper Function Start", 
        "# Gemini CLI Wrapper Function End")?;
    for daemon in daemons {
    ...
    if force {
        if config_dir.exists() {
            info!("Removing unified configuration directory (--force specified): {}", config_dir.display());
            fs::remove_dir_all(config_dir)?;
            info!("Removed configuration directory: {}", config_dir.display());
        } else {
            info!("Configuration directory {} does not exist, skipping removal.", config_dir.display());
        }
        
        // Check for runtime data and remove if force is set
        if let Ok(runtime_dir) = get_runtime_dir() {
            if runtime_dir.exists() {
                 info!("Removing runtime data directory (--force specified): {}", runtime_dir.display());
                 fs::remove_dir_all(&runtime_dir)?;
                 info!("Removed runtime data directory: {}", runtime_dir.display());
            } else {
                info!("Runtime data directory {} does not exist, skipping removal.", runtime_dir.display());
            }
        } else {
         ...
         info!("Configuration directory {} and runtime data were preserved.", config_dir.display());
...
fn update_all(
    ...
    for daemon in daemons {
        if update_daemons || (update_manager_tool && daemon.bin_name == "mcp-hostd") {
            ...
            if daemon.bin_name == "mcp-hostd" { // Always stop mcpd if updating MCP servers
...
fn install_manager(install_dir: &Path) -> Result<()> {
    ...
    if !manager_bin_path.exists() {
        ...
        let output = Command::new("cargo")
            ...
            .output()?;
        if !output.status.success() {
            return Err(anyhow!("Failed to build gemini-manager: {}", String::from_utf8_lossy(&output.stderr)));
        }
    }
...
fn ensure_path_contains(dir_to_add: &Path) -> Result<()> {
    ...
    info!("Install directory {} not found in PATH. Attempting to add it...", dir_str);
    ...
    if shell_config_exists_with_pattern(&shell_config_path, &path_addition_line)? {
        info!("PATH modification for {} already exists in {}.", dir_str, shell_config_path.display());
        return Ok(());
    }
    ...
    info!("Added {} to PATH in {}.", dir_str, shell_config_path.display());
    warn!("You may need to restart your shell or run 'source {}' for changes to take effect.", shell_config_path.display());
...
fn create_daemon_symlinks(install_dir: &Path, daemons: &[DaemonInfo]) -> Result<()> {
    info!("Creating symlinks for daemon binaries...");
        ...
        if daemon.bin_name != daemon.name {
            ...
            if symlink_path.exists() {
                #[cfg(unix)]
                {
                    ...
                    if symlink_path.exists() {
                        info!("Removing existing file/symlink at {}", symlink_path.display());
                        fs::remove_file(&symlink_path)?;
                    }
                    
                    // Create the symlink
                    info!("Creating symlink: {} -> {}", symlink_path.display(), binary_path.display());
                    symlink(&binary_path, &symlink_path)
                        .map_err(|e| anyhow!("Failed to create symlink for {}: {}", daemon.name, e))?;
                }
                
                #[cfg(not(unix))]
                {
                    warn!("Symlink creation not supported on this platform for {}", daemon.name);
