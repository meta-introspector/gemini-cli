    ...
    pub async fn execute_tool(&self, server: &str, tool: &str, args: Value) -> Result<Value> {
        ...
        match response.status {
            ResponseStatus::Success => {
                ...
                match response.payload {
                    gemini_mcp::ipc::ResponsePayload::Result(gemini_mcp::ipc::DaemonResult::ExecutionOutput(output)) => Ok(output),
                    gemini_mcp::ipc::ResponsePayload::Error(err) => Err(anyhow::anyhow!("Daemon returned error for tool execution: {}", err.message)),
                    _ => Err(anyhow::anyhow!("Unexpected response payload format for tool execution")),
                }
            }
            ResponseStatus::Error => {
                // Extract the error message and return it
