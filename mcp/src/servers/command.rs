...
impl CommandServer {
    ...
    pub async fn run(self, stdin: impl AsyncRead + Unpin, mut stdout: impl AsyncWrite + Unpin) -> Result<(), String> {
                            ...
                            if parts.len() == 2 {
                                ...
                                match len_str.parse::<usize>() {
                                    ...
                                    Err(e) => {
                                        ...
                                        let error_resp = format_error_response(0, format!("Invalid Content-Length: {}", e));
    ...
    async fn write_response(&self, stdout: &mut (impl AsyncWrite + Unpin), response: &str) -> Result<(), String> {
    ...
    async fn process_message(&self, message: &str) -> Result<Option<String>, String> {
        ...
        let result = match request.method.as_str() {
            ...
            _ => {
                send_error(&mut buffer, id_value.clone(), -32601, &format!("Method not found: {}", request.method))
            }
        };
...
async fn handle_execute_tool(request: Request, stdout: &mut impl Write) -> Result<(), String> {
    let params: ExecuteToolParams = match request.params {
        Some(p) => serde_json::from_value(p)
            .map_err(|e| format!("Invalid execute tool params: {}", e))?,
        None => return send_error(stdout, request.id.unwrap_or(json!(null)), -32602, "Missing params for tool/execute"),
    };
    
    match params.tool_name.as_str() {
        "execute_command" => execute_command(params.arguments, request.id.unwrap_or(json!(null)), stdout).await,
        "execute_shell" => execute_shell(params.arguments, request.id.unwrap_or(json!(null)), stdout).await,
        _ => send_error(stdout, request.id.unwrap_or(json!(null)), -32601, &format!("Tool not found: {}", params.tool_name)),
    }
}
...
async fn handle_get_resource(request: Request, stdout: &mut impl Write) -> Result<(), String> {
    let params: GetResourceParams = match request.params {
        Some(p) => serde_json::from_value(p)
            .map_err(|e| format!("Invalid get resource params: {}", e))?,
        None => return send_error(stdout, request.id.unwrap_or(json!(null)), -32602, "Missing params for resource/get"),
    };
    
    match params.name.as_str() {
        "os_info" => get_os_info(params.params, request.id.unwrap_or(json!(null)), stdout).await,
        "environment_variables" => get_environment_variables(params.params, request.id.unwrap_or(json!(null)), stdout).await,
        _ => send_error(stdout, request.id.unwrap_or(json!(null)), -32601, &format!("Resource not found: {}", params.name)),
    }
}
...
async fn execute_command(args: Value, id: Value, stdout: &mut impl Write) -> Result<(), String> {
    ...
    let arguments: Vec<String> = args["arguments"].as_array()
        .map(|arr| arr.iter().filter_map(|v| v.as_str().map(String::from)).collect())
        .unwrap_or_default();
    let working_dir = args["working_directory"].as_str();
    let env_vars: HashMap<String, String> = args["environment_variables"].as_object()
    ...
    match cmd.spawn() {
        Ok(child) => {
            match child.wait_with_output() {
                ...
                Err(e) => send_error(stdout, id, -32000, &format!("Failed to read command output: {}", e)),
            }
        }
        Err(e) => send_error(stdout, id, -32001, &format!("Failed to execute command '{}': {}", command_str, e)),
    }
}
    ...
    match cmd.spawn() {
        Ok(child) => {
            match child.wait_with_output() {
                ...
                Err(e) => send_error(stdout, id, -32000, &format!("Failed to read shell output: {}", e)),
            }
        }
        Err(e) => send_error(stdout, id, -32001, &format!("Failed to execute shell command: {}", e)),
    }
}

// Resource implementations
async fn get_os_info(_params: Option<Value>, id: Value, stdout: &mut impl Write) -> Result<(), String> {
...
async fn get_environment_variables(_params: Option<Value>, id: Value, stdout: &mut impl Write) -> Result<(), String> {
...
pub async fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Send immediate initialization response
    {
        ...
        let resources = vec![
            Resource { name: "os_info".to_string(), description: "Gets OS info".to_string(), schema: None },
            Resource { name: "environment_variables".to_string(), description: "Gets env vars".to_string(), schema: None },
        ];
        let capabilities = ServerCapabilities { tools, resources };
        ...
        match serde_json::to_string(&response) {
            Ok(response_json) => {
                ...
                debug!("Sending early init response ({} bytes)", response_json.len());
                ...
                Ok(_) => {
                    ...
                    if line_trimmed.is_empty() {
                        ...
                        if let Some(len_str) = line_trimmed.strip_prefix("Content-Length:") {
                            ...
                            let numeric_part = len_str.chars()
        ...
        if let Some(length) = content_length {
            ...
            match String::from_utf8(buffer) {
                Ok(content) => {
                    ...
                    match process_message(&content, &cmd_executor, &mut stdout).await {
                        ...
                        Err(e) => {
                            ...
                            let error_resp = format_error_response(0, format!("Message processing error: {}", e));
                            write_response(&mut stdout, error_resp).await?;
                        }
                    }
                }
                Err(e) => {
                    ...
                    let error_resp = format_error_response(0, format!("Invalid UTF-8 content: {}", e));
                    write_response(&mut stdout, error_resp).await?;
                }
            }
        } else {
...
// Helper to write response with proper Content-Length framing
async fn write_response(stdout: &mut impl Write, response: String) -> Result<(), Box<dyn std::error::Error>> {
    let message = format!("Content-Length: {}\r\n\r\n{}", response.len(), response);
...
async fn process_message(content: &str, executor: &CommandExecutor, stdout: &mut impl Write) -> Result<(), String> {
    ...
    if let Some(method) = request.get("method").and_then(|m| m.as_str()) {
        ...
        match method {
            "initialize" => {
                ...
                write_response(stdout, response_str).await
                    .map_err(|e| format!("Failed to write response: {}", e))?;
                            ...
                            match tool_name {
                                "execute_command" => {
                                    if let Some(cmd) = arguments.get("command").and_then(|c| c.as_str()) {
                                        ...
                                        let timeout = arguments.get("timeout_ms").and_then(|t| t.as_u64());
                                        ...
                                        match executor.execute_command(cmd, cwd, env, timeout).await {
                                            ...
                                            Err(e) => {
                                                ...
                                                write_response(stdout, error_resp).await
...
impl CommandExecutor {
    ...
    async fn execute_command(&self, command: &str, cwd: Option<&str>, _env: Option<&Value>, _timeout_ms: Option<u64>) -> Result<CommandResult, String> {
