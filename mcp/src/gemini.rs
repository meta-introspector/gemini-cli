...
pub fn sanitize_json_schema(mut schema: Value) -> Value {
...
pub fn build_mcp_system_prompt(tools: &[Tool], resources: &[Resource]) -> String {
    if std::env::var("DEBUG").is_ok() {
        println!(
            "Building MCP system prompt with {} tools and {} resources",
            tools.len(),
            resources.len()
        );
            ...
            )
    {
        prompt.push_str("\n## Memory Storage\n\n");
        prompt.push_str(
            "You have access to a persistent memory storage system. When a user wants to store information for future reference, use the memory storage tools. You should explicitly tell the user when you've stored something in memory.\n\n"
        );
    }
    ...
    prompt.push_str(
        "Important: Always use the function calling capability of the API, not text-based suggestions. DO NOT write code blocks with JSON in your response."
    );
